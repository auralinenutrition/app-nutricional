...
function Signup() {
    ...
    const handleSignup = async ()=>{
        try {
            ...
            // valida em runtime — isso garante ao TypeScript que userId é string após o if
            if (!userId) {
                // se não tiver ID, tenta obter sessão (fallback)
                const sessionRes = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].auth.getSession();
                const fallbackId = sessionRes?.data?.session?.user?.id;
                if (!fallbackId) {
                    throw new Error("Não foi possível obter o ID do usuário após o cadastro.");
                }
                // se encontrou, usa o fallbackId
                // (isso faz com que TypeScript saiba que userIdReal é string)
                const userIdReal = fallbackId;
                // montar payload (como você já faz)
                const payload = {
                    ...quizData,
                ...
                const saveRes = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$services$2f$quizService$2e$tsx__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["default"].saveQuizResponses(userIdReal, payload);
            // tratar resultado...
            } else {
                // userId está definido e o TypeScript entende como string aqui
                const payload = {
                    ...quizData,
                    peso_atual: Number(quizData.peso_atual),
                    altura: Number(quizData.altura),
                ...
                const saveRes = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$services$2f$quizService$2e$tsx__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["default"].saveQuizResponses(userId, payload);
                if (!saveRes.success) {
                    // tentar inserir manualmente como fallback
                    const fallback = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from("quiz_responses").insert({
                        user_id: userId,
                        objetivo: payload.objetivo,
                        nivel_treino: payload.nivel_treino,
                        frequencia_treino: payload.frequencia_treino,
                ...
                const { error: updateErr } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from("users").update({
                    peso_atual: payload.peso_atual,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__78d035e4._.js.map